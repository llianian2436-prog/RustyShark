...
fn export_to_pcap(raw_packets: &[Vec<u8>]) -> std::io::Result<()> {
    ...
    let usecs = now.subsec_micros(); // 已移除多余的 as u32 转型
    ...
    loop {
        if should_redraw {
            terminal.draw(|f| {
                ...
                // 局部允许 needless_range_loop 保证绝对行号映射模型可读性
                #[allow(clippy::needless_range_loop)]
                for idx in start..end {
                    let raw_data = &raw_packets[idx];
                    let item_color; // 移除冗余赋值警告
                                ...
                                parser::IpProtocol::Ipv4(ip) => {
                                    ...
                                    match ip.transport {
                                        parser::TransportProtocol::Tcp(tcp) => {
                                            ...
                                            // 优化静态字符串渲染，杜绝无效 format! 警告
                                            tree.push_str(" └─ [Layer 4] 传输控制协议 (TCP)\n");
                                        ...
                                        parser::TransportProtocol::Udp(udp) => {
                                            tree.push_str(" └─ [Layer 4] 用户数据报协议 (UDP)\n");
                ...
                let details_panel = Paragraph::new(details_text)
                    .block(Block::default().title(" 协议树深度明细 ").borders(Borders::ALL).border_style(Style::default().fg(Color::Green)));
                ...
                let tip_text = if info_message.is_empty() {
                    " 提示：使用鼠标左键点击或按住拖拽右侧滚动条。"
                } else {
                    &info_message
                };
                let tip_msg = Paragraph::new(format!("  状态反馈: {}", tip_text))
                ...
                match ev {
                    ...
                    Event::Mouse(mouse) => {
                        match mouse.kind {
                                ...
                                if let Ok(size) = terminal.size() {
                                       ...
                                       && mouse.column > list_chunk.x && mouse.column < list_chunk.x + list_chunk.width - 1
                                    {
                                        ...
                                        if mouse.row >= btn_bar_chunk.y && mouse.row < btn_bar_chunk.y + btn_bar_chunk.height {
                                            ...
                                            // 折叠紧凑条件分支，满足 collapsible_if 审计
                                            let b_pause = sub_btn_chunks[2];
                                            if click_col >= b_pause.x && click_col < b_pause.x + b_pause.width && capture_state == CaptureState::Running {
                                            ...
                                            if click_col >= b_clear.x && click_col < b_clear.x + b_clear.width {
                                                if capture_state != CaptureState::Running {
                                                    ...
                                                    info_message = "警告：请先暂停或停止抓包再清空数据！".to_string();
                                                }
                                            }
                            ...
                            // 折叠嵌套匹配，升级为高级 match 条件守卫（满足 collapsible_match）
                            MouseEventKind::ScrollUp if selected_idx > 0 => {
                                selected_idx = selected_idx.saturating_sub(1);
                            }
                            MouseEventKind::ScrollDown if !raw_packets.is_empty() && selected_idx < raw_packets.len() - 1 => {
                                selected_idx += 1;
                            }
                            _ => {}
                        }
